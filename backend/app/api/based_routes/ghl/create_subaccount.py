...
from backend.app.go_high_level_home.api.sub_accounts.sub_account.put import (
    update_sub_account,
)
...
async def create_subaccount_logic(req: SubAccountCreateRequest):
    try:
        result = await update_sub_account(req.location_id, req.headers, req.sub_account_data)
        return result
